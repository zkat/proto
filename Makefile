...
# Files
#
main-file = src/$(project-name).js
source-files = 
build-dir = build
docs-dir = docs
examples-dir = examples
browserify-bundle = $(build-dir)/$(project-name).js
min-file = $(build-dir)/$(project-name).min.js
source-map-filename = $(project-name).js.src
...
.PHONY: release-%
release-%: all
	npm version $* -m "Upgrading $(project-name) to %s" ;
	git checkout master ; \
